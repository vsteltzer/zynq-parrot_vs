...
#ifndef BP_ZYNQ_AARCH64_PL_H
#define BP_ZYNQ_AARCH64_PL_H
...
using namespace bsg_nonsynth_dpi;
  ...
  typedef uint64_t axil_addr_uint;
  #define AXIL_ADDR_PRINT PRIu64
#elif ARCHITECTURE == __arm__
  typedef unsigned int axil_addr_uint;
  #define AXIL_ADDR_PRINT %x
#endif
...
public:
  ...
  void axil_write(axil_addr_uint address, int data, int wstrb) {
    axil_addr_uint address_orig = address;
    int index;
    ...
    bsg_pr_dbg_pl("  bp_zynq_pl: AXI writing [%" AXIL_ADDR_PRINT "] -> port %d, [%" AXIL_ADDR_PRINT "]<-%8.8x\n",
  ...
  int axil_read(axil_addr_uint address) {
    axil_addr_uint address_orig = address;
    int index = 0;
    int data;
    ...
    // we subtract the bases to make it consistent with the Zynq AXI IPI
    // implementation
    // bsg_pr_info("ps.cpp: Address: %" AXIL_ADDR_PRINT " \n", address);
    if (address >= GP0_ADDR_BASE &&
        address <= GP0_ADDR_BASE + GP0_ADDR_SIZE_BYTES) {
      index = 0;
      address = address - GP0_ADDR_BASE;
    ...
    bsg_pr_dbg_pl("  bp_zynq_pl: AXI reading [%" AXIL_ADDR_PRINT "] -> port %d, [%" AXIL_ADDR_PRINT "]->%8.8x\n",
                  address_orig, index, address, data);
